...
struct ResticIntegrationTests {
    ...
    @Test("cancelling a running backup terminates restic and reports .cancelled")
    func cancellation() async throws {
        let fixture = try makeFixture()
        defer { try? FileManager.default.removeItem(at: fixture.root) }
        _ = try await fixture.service.initializeRepository(fixture.context)

        // The backup must still be running when we cancel it, with a comfortable
        // margin: 2 GiB of fresh, incompressible data takes several seconds even
        // on fast hardware. Compressible data can finish in well under a second
        // — the original version of this test backed up 120 MB of it and would
        // have passed silently on a machine fast enough to finish before the
        // cancel. The dataset size, not restic's output, is what makes the
        // timing deterministic here.
        let bulk = fixture.sourceDirectory.appendingPathComponent("bulk")
        ...
        for _ in 0 ..< 64 {
            ...
            try chunk.write(to: bulk.appendingPathComponent("f-\(UUID().uuidString).bin"))
        }
        ...
        // Cancelling one second in leaves seconds of work on the table on any
        // plausible machine. If the backup still *completed*, the dataset no
        // longer outlasts the delay and the test must fail rather than pass
        // without having exercised cancellation at all.
        do {
            _ = try await task.value
            Issue.record("backup completed even though it was cancelled mid-run")
        } catch let error as ResticError {
            #expect(error == .cancelled)
        }
    }
    @Test("backup progress is delivered while the backup is still running")
    func progressArrivesMidRun() async throws {
        let fixture = try makeFixture()
        defer { try? FileManager.default.removeItem(at: fixture.root) }
        _ = try await fixture.service.initializeRepository(fixture.context)

        // Regression lock: the pipe reader once buffered restic's status lines
        // until process exit (Foundation's FileHandle.read on a pipe), so the
        // progress UI sat at 0% for the whole backup and jumped to done at the
        // end. The dataset must be big enough that a healthy run spans several
        // one-second status intervals.
        let bulk = fixture.sourceDirectory.appendingPathComponent("bulk")
        try FileManager.default.createDirectory(at: bulk, withIntermediateDirectories: true)
        for _ in 0 ..< 64 {
            var chunk = Data(count: 32 * 1024 * 1024)
            chunk.withUnsafeMutableBytes { buffer in
                arc4random_buf(buffer.baseAddress, buffer.count)
            }
            try chunk.write(to: bulk.appendingPathComponent("f-\(UUID().uuidString).bin"))
        ...
        _ = try await service.backup(context, plan: plan) { _ in
            firstProgress.mark()
        }
        let elapsed = Date.now.timeIntervalSince(startedAt)

        guard let at = firstProgress.value else {
            Issue.record("no progress was reported at all")
            return
        }
        // When the reader buffers, every status line arrives in one flush at
        // process exit and the first one lands at ~100% of the run. A healthy
        // run reports its first status seconds before finishing (~30% here).
        // Ratio, not a fixed gap, so the assertion holds on any machine speed.
